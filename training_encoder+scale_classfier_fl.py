...
from model_scale_classifier_fl import EncodecModel 
...
SAVE_FOLDER = f'/data2/xintong/EnCodec_Finetune/encoder_only+scale_classifier_fl/'
...
def training(max_epoch = 5, log_interval = 20, fixed_length = 0, tensor_cut=100000, batch_size=8):
    ...
    def train_classifier(epoch):
        ...
        for batch_idx, (input_wav, f0, uv, tgt) in enumerate(trainloader):
            ...
            loss_tgt, loss_f0, loss_uv = model(input_wav, f0, uv, tgt, "encoder")
            ...
            if batch_idx % log_interval == 0:
                ...
                print(f"Train Epoch: {epoch} [{batch_idx * len(input_wav)}/{len(trainloader.dataset)} ({100. * batch_idx / len(trainloader):.0f}%)], loss_all {loss_tgt.item()}, loss_f0: {loss_f0.item()}, loss_uv: {loss_uv.item()}")
