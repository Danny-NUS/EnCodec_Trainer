...
from model_full_scale_grl import EncodecModel 
...
SAVE_FOLDER = f'/data2/xintong/EnCodec_Finetune/encoder_decoder_scale_grl/'
...
def training(max_epoch = 5, log_interval = 20, fixed_length = 0, tensor_cut=100000, batch_size=8):
    ...
    def train(epoch):
        ...
        for batch_idx, (input_wav, f0, uv, tgt) in enumerate(trainloader):
            ...
            disc.zero_grad()
            output, loss_enc, _, loss_qv_penalty, loss_qv_quant, loss_f0, loss_uv = model(input_wav, f0, uv, tgt, "full")

            logits_real, fmap_real = disc(input_wav)
            if train_d:
                logits_fake, _ = disc(model(input_wav, f0, uv, tgt, "full")[0].detach())
            ...
            if batch_idx % log_interval == 0:
                ...
                print(f"Train Epoch: {epoch} [{batch_idx * len(input_wav)}/{len(trainloader.dataset)} ({100. * batch_idx / len(trainloader):.0f}%)], loss_enc: {loss_enc.item()}, loss {loss.item()}, loss_qv_penalty: {loss_qv_penalty}, loss_qv_quant: {loss_qv_quant}, loss_f0: {loss_f0.item()}, loss_uv: {loss_uv.item()}")
    ...
    checkpoint_path = "/data2/xintong/EnCodec_Finetune/encoder_only+scale+grl/batch5_cut48000_epoch38.pth"
    ...
    # load_info = model.decoder.load_state_dict(decoder_state_dict, strict=False)
